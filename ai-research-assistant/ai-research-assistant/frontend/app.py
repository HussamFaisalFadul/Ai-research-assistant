...
#cv{{width:100%;height:650px;display:block;cursor:grab}}
...
<script>
const DATA = {json_str};
const COLS = ['#5b6ef5','#22c55e','#f59e0b','#8b5cf6','#06b6d4','#ec4899','#ef4444','#10b981','#6366f1','#f97316'];
...
function drawText(g, box, x, y) {{
  const {{lines, fs, lh}} = box;
  const startY = y - (lines.length * lh) / 2 + lh * 0.7;
  lines.forEach((l, i) => {{
    g.append('text')
...
function layoutTree(data, W, H) {{
  ...
  const root = {{
    ...
    box: rootBox, color: '#5b6ef5', topic: data.topic
  }};
  nodes.push(root);
  ...
  const R1 = isMobile
    ? base * (0.35 + Math.min(n * 0.04, 0.25))
    : base * (0.32 + Math.min(n * 0.02, 0.15));
  ...
  children.forEach((child, ci) => {{
    ...
    const cNode = {{
      ...
      box: cBox, color: col, topic: child.topic, parentId: 0
    }};
    nodes.push(cNode);
    ...
    const R2 = isMobile
      ? base * (0.22 + Math.min(ns * 0.03, 0.12))
      : base * (0.2 + Math.min(ns * 0.02, 0.1));
    ...
    subs.forEach((sub, si) => {{
      ...
      nodes.push({{
        ...
        box: sBox, color: col + '99', topic: sub.topic, parentId: cNode.id
      }});
...
document.getElementById('zm').onclick = () => svg.transition().duration(250).call(zoom.scaleBy, 0.7);
document.getElementById('zp').onclick = () => svg.transition().duration(250).call(zoom.scaleBy, 1.4);
document.getElementById('zr').onclick = () => svg.transition().duration(350).call(zoom.transform, d3.zoomIdentity);

let resizeTimeout;
window.addEventListener('resize', () => {{
  clearTimeout(resizeTimeout);
  resizeTimeout = setTimeout(() => initializeAndDraw(), 250);
}});
