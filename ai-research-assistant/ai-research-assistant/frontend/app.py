...
with tab2:
    ...
    if st.session_state.mm_step == 0:
        ...
        for idx, src in enumerate(NEWS_SOURCES):
            with all_btn_cols[idx]:
                if st.button(src["label"], key=f"news_btn_{src['key']}", use_container_width=True):
                    with st.spinner(f"⏳ جاري جلب {src['label']}..."):
                        if src["type"] == "weather":
                            fetched = fetch_weather_alkhobar()
                        else:
                            fetched = fetch_rss_news(src["url"], src["label"])
                    
                    if fetched.startswith("❌") or fetched.startswith("⏳"):
                        st.error(fetched)
                    else:
                        st.session_state.mm_raw_text = fetched
                        st.success(f"✅ تم جلب {src['label']} — اضغط «تحليل وبناء الخريطة»")
                        st.rerun()
